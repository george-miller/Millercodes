...
def main():
	...
	#-------Create the Paddle-------#
	my_paddle = Paddle(screen, RED, (100, 20), (350, 600))
	...
	#------Create the Ball---------#
	ball = Gameball(screen, WHITE, 8, (5,-5), my_paddle)
	...
	while not done:
		...
		if lives > 0:
			...
			if len(ball_list) == 0: #If there are no balls left
				lives -= 1
				player_ball = Gameball(screen, WHITE, 8, (5,-5), my_paddle) #Create a new ball
			...
			for player_ball in ball_list: #Check each ball in the game
				...
				if not player_ball.launched:
					player_ball.holdpaddle(my_paddle) #Have the ball follow the paddle
				...
				for brick in brick_hit_list:
					if player_ball.hit(brick): #Check to make sure the ball actualy hit the brick
					#### There are still bugs with the mask colliions where the ball will not 
					#### be touching a brick that shows up on the list.
