...
class Super_State():
	...
	def draw_rects(self, button_rects, button_labels, highlight, button_color):
		...
		for i in range(len(button_rects)):
			...
			text = font.render(str(self.button_labels[i]), True, color)
			button.blit(text, (self.button_rects[i].width/2 - text.get_rect().width/2, self.button_rects[i].height/2 - text.get_rect().height/2))
			self.g.screen.blit(button, (self.button_rects[i].x, self.button_rects[i].y))
	...
	def draw(self):
		...
		for i in range(len(self.button_rects)):
			if i in self.g.completed:
				completed_rects.append(self.button_rects[i])
...
class Main_Game(Super_State):
	...
	def prepare(self):
		#set the level according to the state
		if self.g.level == 0:
			level_design.regenerate()
		self.level = Level(**level_design.LEVEL_DESIGN[self.g.level])
	...
	def update(self):
		...
		for event in self.g.events:
			if event.type == pygame.QUIT:
				...
				if self.g.mouse[1]-self.textheight < 0 and self.g.mouse[0] > self.g.screen.get_width()/8:
					self.g.state = 0
					self.g.level = 0
				else:
					self.level.turnTile(self.g.mouse[0], self.g.mouse[1]-self.textheight, self.ctrl) #Turn the tile clicked
					self.level.calcPower() #Recalculate power for level
					if self.level.done: #If you finished the level
						self.g.score += self.level.getScore() #Add score
						self.level.moves = 0 #Remove remaining moves
						self.draw()
						pygame.display.flip()
						self.g.pause(30)
						if self.g.level != 0:
							self.g.completed.append(self.g.level)
						self.g.state = 5
						if self.g.level == 0:
							self.g.state = 4
				self.g.dirty = True
	...
	def draw(self):
		self.g.screen.fill(WHITE)
		self.textheight = fullboard(self.g.screen, self.g.score, self.level)
		self.g.dirty = False
